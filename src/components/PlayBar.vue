...
import {setCollection, download, isCollected} from '../api/index'
...
export default {
  ...
  watch: {
    ...
    isCollected (val) {
      if (val) {
        this.$store.commit('setStarButtonUrl', 'el-icon-star-off')
      } else {
        this.$store.commit('setStarButtonUrl', 'el-icon-star-on')
      }
    },
    volume () {
  ...
  onload:{
    isCollected(isCollectedParams){
      isCollected(isCollectedParams).then(res=>{
        if(res.code===1) {
          this.$store.commit('setIsCollected', true)
        }else{
          this.$store.commit('setIsCollected', false)
        }
      })
    }
  },
