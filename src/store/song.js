const song={
  state:{
    ...
    starButtonUrl:'el-icon-star-off',//收藏状态的图标
  },
  getters:{
    isPlay: state=>state.isPlay,
    isCollected: state=>state.isCollected,
    playButtonUrl: state=>{
    ...
    picUrl: state => {
      ...
      if (!picUrl) {
        picUrl = JSON.parse(window.sessionStorage.getItem('picUrl')) || 'http://47.118.78.168:8888/img/songPic/tubiao.jpg'
      }
      return picUrl
    },
    autoNext: state => {
  ...
  mutations:{
    ...
    setIsCollected: (state, isCollected) => {
      state.isCollected = isCollected
      window.sessionStorage.setItem('isCollected', JSON.stringify(isCollected))
    },
    ...
    setStarButtonUrl: (state, starButtonUrl) => {
      state.starButtonUrl = starButtonUrl
      window.sessionStorage.setItem('starButtonUrl', JSON.stringify(starButtonUrl))
    },
    setId: (state, id) => {
      state.id = id
